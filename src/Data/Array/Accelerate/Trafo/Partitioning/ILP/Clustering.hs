...
topSort (Graph allLabels fedges fpedges) cluster = map ExecL topsorteds
  where
    buildGraph =
            G.graphFromEdges
          . map (\(a,b) -> (a,a,b))
          . M.toList
          . flip (S.fold (\(x :-> y) -> M.adjust (y:) x)) (fedges <> fpedges)
          . M.fromList
          . map (,[])
          . S.toList

    (fullGraph, fullNodeFromVertex, _) = buildGraph allLabels
    topsorteds = map (:[]) $ filter (`S.member` cluster) $ map (\(a,_,_)->a) $  map fullNodeFromVertex $ G.topSort fullGraph
